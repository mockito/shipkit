...
\t]
    ...
    private DependencySet dependencies() {
        ...
        def notSelfResolving = Mock(Dependency)

        def emptyFiles = Mock(SelfResolvingDependency)
        emptyFiles.resolve() >> new HashSet()

        def selfResolving = Mock(SelfResolvingDependency)
        selfResolving.resolve() >> ( [new File("test"), new File("test2")] as Set)

        def dependencyIterator = Mock(Iterator)
        dependencyIterator.hasNext() >>> [true, true, true, false]
        dependencyIterator.next() >>> [notSelfResolving, emptyFiles, selfResolving]
        dependencySet.iterator() >> dependencyIterator
    ...
    private Set<ResolvedArtifact> resolvedArtifacts() {
        ...
        //def moduleVersion = moduleVersion("depGroup", "depName", "depVersion")
